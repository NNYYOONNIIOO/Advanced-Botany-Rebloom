...
public class RecipeAdvancedPlate {
    ...
    public boolean matches(IInventory inv) {
        ...
        for (int i = 1; i < inv.getSizeInventory() && (stack = inv.getStackInSlot(i)) != null; ++i) {
            int stackIndex = -1;
            for (int j = 0; j < inputsMissing.size(); ++j) {
                ItemStack input = (ItemStack)inputsMissing.get(j);
                if (!(input instanceof ItemStack) || !this.simpleAreStacksEqual(input.copy(), stack)) continue;
                stackIndex = j;
                break;
            }
            if (stackIndex == -1) {
